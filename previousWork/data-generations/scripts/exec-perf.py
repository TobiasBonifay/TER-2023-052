...
def run_benchmark(start_time, reduction_value, measurement):
    ...
    while (True):
        ...
        if t == n:
            ...
            os.system("ab -n 100000 -c 500 -e " + dir_bench + "/" + format_bench_name(str(t), t,
                                                                                      10000) + ".csv http://192.168.122.49:80/")
            n += 45
            b += 1
...
def change_limit_cgroup_file(cgroup_limit):
    with open(r"/sys/fs/cgroup/machine.slice/machine-qemu\x2d6\x2dubuntu22.04.scope/memory.max", "w") as fmax:
        print("change limit to " + cgroup_limit)
    ...
    for reduction_value in [0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2]:
        for measurement in [i for i in range(5)]:
            ...
            client_thread = threading.Thread(target=run_client,
                                             args=(client, start_time, reduction_value, measurement, dir_output))
            client_thread.start()
